...
class RefreshAccessUseCase(UseCaseInterface):
    ...
    def process_request(self, request_dict: dict):
        try:
            ...
            if uid:
                authentication_data = self.user_auth_repository.get_authentication_data(
                    uid=uid
                )
                response_data = {"result": authentication_data, "http_status_code": 200}

            else:
                authentication_data = self.user_auth_repository.refresh_access(
