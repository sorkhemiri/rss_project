...
class UserSubscriptionsListUseCase(UseCaseInterface):
    ...
    def process_request(self, request_dict: dict):
        try:
            ...
            offset = data.offset
            limit = data.limit
            subscriptions_data = self.subscription_repository.get_user_subscriptions_list(user_id=user.id, offset=offset, limit=limit)
            subscriptions_list = [item.dict(exclude_defaults=True) for item in subscriptions_data]
