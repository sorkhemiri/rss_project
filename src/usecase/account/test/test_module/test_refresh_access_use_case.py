...
class RefreshAccessUseCaseTestCase:
    ...
    def test_input():
        ...
        request_data = {"access_token": "fake_access_token"}

        data = use_case.execute(request_model=request_data or {})
        assert data["http_status_code"] == 400
        assert data["type"] == "VALIDATION ERROR"

        request_data = {"refresh_token": "fake_refresh_token"}
        data = use_case.execute(request_model=request_data or {})
        assert data["http_status_code"] == 400
        assert data["type"] == "VALIDATION ERROR"
    ...
    def test_invalid_refresh_token(
        access_token_not_valid_patch, refresh_token_not_valid_patch
    ):
        use_case = RefreshAccessUseCase(
            validator=RefreshAccessValidator, user_auth_repository=UserAuthRepository
        )
    ...
    @staticmethod
    def test_use_of_refresh_token(
        access_token_not_valid_patch, refresh_token_valid_patch
    ):
        use_case = RefreshAccessUseCase(
            validator=RefreshAccessValidator, user_auth_repository=UserAuthRepository
        )
